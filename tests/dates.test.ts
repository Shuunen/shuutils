...
it('dateIso10 specific date', () => { expect(dateIso10(new Date('2018-11-30'))).toBe('2018-11-30') })
it('dateIso10 today', () => { expect(dateIso10(today)).toBe(today.toISOString().split('T')[0]) })

// use dateIso10 to remove milliseconds that would differ from two new Date() instances
it('daysAgo give today by default', () => { expect(dateIso10()).toBe(dateIso10(daysAgo())) })

it('daysAgoIso10 give today by default', () => { expect(daysAgoIso10()).toBe(dateIso10(today)) })

it('iso string date has length', () => { expect(dateToIsoString(new Date()).length > 0).toBe(true) })
it('iso string date contains timezone by default', () => { expect(dateToIsoString(new Date()).toLowerCase().includes('z')).toBe(true) })
it('iso string date does not contains timezone if needed', () => { expect(dateToIsoString(new Date(), true).toLowerCase().includes('z')).toBe(false) })
...
it('format date as d', () => { expect(formatDate(date, 'd')).toBe('2') })
it('format date as dd', () => { expect(formatDate(date, 'dd')).toBe('02') })
it('format date as dd/MM/yyyy HH:mm:ss', () => { expect(formatDate(date, 'dd/MM/yyyy HH:mm:ss')).toBe('02/09/2021 07:08:28') })
it('format date as dd/MM/yyyy', () => { expect(formatDate(date, 'dd/MM/yyyy')).toBe('02/09/2021') })
it('format date as eee d', () => { expect(formatDate(date, 'eee d')).toBe('Thu 2') })
it('format date as eee', () => { expect(formatDate(date, 'eee')).toBe('Thu') })
it('format date as eeee', () => { expect(formatDate(date, 'eeee')).toBe('Thursday') })
it('format date as HH', () => { expect(formatDate(date, 'HH')).toBe('07') })
it('format date as mm', () => { expect(formatDate(date, 'mm')).toBe('08') })
it('format date as MM', () => { expect(formatDate(date, 'MM')).toBe('09') })
it('format date as MMMM', () => { expect(formatDate(date, 'MMMM')).toBe('September') })
it('format date as ss', () => { expect(formatDate(date, 'ss')).toBe('28') })
it('format date as yy-MM-dd', () => { expect(formatDate(date, 'yy-MM-dd')).toBe('21-09-02') })
it('format date as yy', () => { expect(formatDate(date, 'yy')).toBe('21') })
it('format date as yyyy (space)', () => { expect(formatDate(date, 'yyyy ')).toBe('2021 ') })
